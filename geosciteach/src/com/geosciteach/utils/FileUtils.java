...
public class FileUtils {
	 ...
	 *         (true) or not (false)
	 */
    public static boolean checkExternalStorageReadAndWritable(Context baseContext) {
        ...
        if (!mExternalStorageWritable) {
            Toast.makeText(baseContext, baseContext.getString(R.string.no_external_writable),
	...
	public static String getUniqueFileNameAtApplicationDirectory(String filename, Context baseContext){
		...
		while(file.exists()){
			counter++;
			file = new File(dir, fileNameWithOutExtension + counter + fileExtension);
		}
	...
	public static File prepareFileToWriteDetailsTo(Context context, String filename) {
        ...
        if(uniqueUser==null){
        	...
        	fileToWriteTo = new File(FileUtils.getApplicationDirectory(context) + filename);
        }
        else{
        	fileToWriteTo = new File(FileUtils.getApplicationDirectory(context) + uniqueUser + filename);
        }
	...
	public static String getUniqueUser(Context context){
		SharedPreferences loginDetails = context.getSharedPreferences(PasswordEntryActivity.LOGIN, context.MODE_PRIVATE);
		String uniqueuser = loginDetails.getString(PasswordEntryActivity.UNIQUEUSER, null);
